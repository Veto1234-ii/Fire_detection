...
# folder = r'F:\Gis\fire\testLandsat\LC08_L1TP_'+X+'_01_T1'
...
np.save('fire_mask_'+X,result)                                         
print(np.max(result))
print(np.sum(result)) 

#

# def isWater(b1, b2, b3, b4, b5, b6, b7):
#     if(b4 > b5 and b5 > b6 and b6 >b7 and b1-b7 < 0.2 and (b3 > b2 or b1 > b2 and b2 > b3 and b3 > b4)):
#         return True
#     return False

# def isDefinatelyFire(b1,b5,b6,b7):
#     if b7>0.5 and b7/b5 > 2.5 and b7-b5 > 0.3:
#         return 1
#     if b6 > 0.8 and b1 < 0.2  and (b5 > 0.4 or b7 < 0.1):
#         return 2
#     return 0

# def Avg_and_std(band):
#     avg = np.average(band)
#     std = np.std(band)
#     return (avg,std)
...
#     for j in range(shape[1]):
#         if data[i][j]==1:
#             r = isDefinatelyFire(b_1s[i][j], b_5s[i][j], b_6s[i][j], b_7s[i][j])
#             result[i][j] = r
#             if(r > 0):
#     #            print(str(i) + " "+ str(j))
#                 continue
            
#             water = isWater(b_1s[i][j], b_2s[i][j], b_3s[i][j], b_4s[i][j], b_5s[i][j], b_6s[i][j], b_7s[i][j])
      
            
#             if(b_7s[i][j] > 0 and b_7s[i][j]/b_5s[i][j] > 1.8 and b_7s[i][j]-b_5s[i][j] > 0.17 and water==False and b_7s[i][j]/b_6s[i][j] > 1.6):
#     #            print('test3')
    
#                 b_5_61=b_5s[i-30:i+31,j-30:j+31]
#                 b_7_61=b_7s[i-30:i+31,j-30:j+31]
                               
#                 R75 = np.divide(b_7_61, b_5_61)
#     #            print(R75)
#                 Sr_7=np.mean(b_7_61)
#                 Sr_5=np.mean(b_5_61)
#                 Sr7_5=Sr_7/Sr_5
#                 Stand7=np.std(b_7_61)
                
                
                        
#                 Stand75=np.std(R75)
                        
#                 if b_7s[i][j] > Sr_7 + max(3*Stand7,0.08):
#                     if b_7s[i][j] / b_5s[i][j] > Sr7_5 + max(3*Stand75,0.8):
#                         result[i][j]=3
