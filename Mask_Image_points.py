...
def Save_mask_image_points(firemask, info, path_res):
    
    res = np.load(firemask)
    
    shape = res.shape
    Reshape_image(path_res, shape, info)
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            if res[i][j]==1:
                ...
                mask[i-50:i+51, j-50:j+51] = 30
    
    imageio.imwrite(path_res + '\img_'+info+'.jpg', img)
    imageio.imwrite(path_res + '\mask_'+info+'.jpg', mask)
